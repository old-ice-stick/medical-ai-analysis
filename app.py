...
# 2. 路径处理 (使用 pathlib 自动处理 Linux/Windows 路径差异)
...
if model is None:
    ...
    st.info("提示：如果是刚上传文件，请点击右上角 'Settings' -> 'Clear Cache' 然后刷新页面。")
    st.stop()
...
if uploaded_file:
    ...
    if final_kps is None:
        ...
        if rot_name != "原始方向":
            st.info(f"✅ 自动识别成功：已将图片旋转 **{rot_name}** 以匹配模型。")

        # 处理器选择 (对应 logic.py 中的类)
        ...
        with col1:
            st.image(cv2.cvtColor(final_img, cv2.COLOR_BGR2RGB), caption="分析用图", use_container_width=True)
        with col2:
            # 绘制关键点总览图
            overview = final_img.copy()
            for i, pt in enumerate(final_kps, 1):
                if pt[0] > 0 and pt[1] > 0: # 过滤掉未检测到的点
                    cv2.circle(overview, (int(pt[0]), int(pt[1])), 5, (0, 255, 0), -1)
            st.image(cv2.cvtColor(overview, cv2.COLOR_BGR2RGB), caption="关键点定位总览", use_container_width=True)
